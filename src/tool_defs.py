...
from src.subagent import spawn_subagent
from src.teammate import spawn_teammate_thread
from src.message_bus import BUS
from src.protocol import consume_lead_inbox, pending_requests, ProtocolState, new_request_id
from src.cron_scheduler import run_schedule_cron, run_list_crons, run_cancel_cron
from src.mcp import connect_mcp
from src.tools import run_bash, run_read, run_write, run_edit, run_glob, run_todo_write
...
def run_connect_mcp(name: str, command: list | str = None) -> str:
    return connect_mcp(name, command)
...
BUILTIN_TOOLS = [
    ...
    {"name": "keep_worktree",
     ...
     "input_schema": {"type": "object",
                      ...
                      "required": ["name"]}},
    {"name": "connect_mcp",
     "description": "Connect to an MCP server via command and discover tools. "
                    "Example command: ['npx', '-y', '@modelcontextprotocol/server-github']. "
                    "MCP tools are prefixed mcp__{server}__{tool}.",
     "input_schema": {"type": "object",
                      "properties": {
                          "name": {"type": "string",
                                   "description": "Server name for reference"},
                          "command": {"type": "array",
                                      "items": {"type": "string"},
                                      "description": "Command + args to start the MCP server"},
                      },
